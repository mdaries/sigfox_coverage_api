...
# TO DO
# Add param mgmt to script with option (dbg,osm,google,)
# add param for device class + indoor/outdor/deep indoor
# ERROR HTTP response mgmt
...
#input_csv = os.path.join(current_folder,"location_ireland.csv")
...
#input_csv = os.path.join(current_folder,"test.txt")
...
#Read credentials and calls test_cred
...
def test_cred(cred_name,request_type,cred):
    #if cred["login"] is not None and cred["password"] is not None:
                ...
                for row in csv_reader:
                    ...
                    try:
                        ...
                        if is_coord(row[0],row[1],line_count):
                            ...
                            errors+=1
                            #logging.error(f"Line {line_count+1}: incorrect value {row[0]},{row[1]}")
                        line_count += 1
                    except IndexError:
...
async def fetch_coverage_async(url,session, lat, lng, radius=200):
    ...
    async with session.get(url,params=params) as response:
        ...
        dict_result = {"pos":[lat,lng],"covered":covered,"margins":margins_offset,"lqi":lqi}        #{'pos': ['-34.921403', '-54.945659'], 'covered': True, 'margins': [18, 0, 0],'lqi':3}
        
    return dict_result

async def bound_fetch(sem,url,session,lat,lng,radius=200):
    # Getter function with semaphore.
    async with sem:
        return await fetch_coverage_async(url,session,lat,lng,radius)
